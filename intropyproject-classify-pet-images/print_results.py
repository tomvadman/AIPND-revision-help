...
def print_results(results_dic, results_stats_dic, model, 
    ...
    #
    # loop results_stats_dic, filter out 'pct' group.
    # split key into list and format as above '% Correct Dogs'
    for key in results_stats_dic:
        # only use keys that starts with pct.
        if key.startswith( 'pct' ):
            
            # remove pct_ and split on _
            key_list = str(key[4:]).split("_")
            
            # 'match' get index out of bounce. Take only list where len(list) > 1.
            if len(key_list) > 1:
    ...
    if (print_incorrect_dogs and 
        ...
        for key in results_dic:
            ...
            # Solution
            #
            # Add some friendlyName variables
            pet_match_classifier = results_dic[key][2]
